        ...
        <div class="results-container">
            ...
            <div style="margin-top: 40px; text-align: left;">
                ...
                ${sortedResults.map((result, idx) => `
                    <div class="question-container" style="margin-bottom: 25px; ${result.isCorrect ? 'border-left-color: #10b981;' : 'border-left-color: #ef4444;'}">
                        <div style="display: flex; align-items: center; margin-bottom: 15px;">
                            <span style="background: ${result.isCorrect ? '#10b981' : '#ef4444'}; color: #ffffff; padding: 4px 12px; font-weight: 600; margin-right: 15px;">
                                ${result.isCorrect ? '✓ Correct' : '✗ Incorrect'}
                            </span>
                            <span style="color: #9ca3af; font-size: 0.9em;">Question ${questionResults.indexOf(result) + 1}</span>
                        </div>
                        <div class="question-text" style="margin-bottom: 20px;">${result.question}</div>
                        <div style="margin-bottom: 10px;">
                            <strong style="color: ${result.isCorrect ? '#10b981' : '#ef4444'};">
                                ${result.isCorrect ? 'Your Answer (Correct):' : 'Your Answer:'}
                            </strong>
                        </div>
                        <div class="answer-option ${result.isCorrect ? 'correct' : 'incorrect'}" style="margin-bottom: 15px;">
                            <label class="answer-label">${result.options[result.userAnswer]}</label>
                        </div>
                        ${!result.isCorrect ? `
                            ...
                            <div class="answer-option correct" style="margin-top: 10px;">
                                <label class="answer-label">${result.options[result.correctAnswer]}</label>
                            </div>
                        ` : ''}
                    </div>
                `).join('')}
